  ...
  fetch('https://data.nasa.gov/resource/gvk9-iz74.json')
    ...
    .then(data => {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        fetch(
          `https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lon}&units=imperial&appid=98e506b01a8b3e4f4d59a0b87727bcec`
        )
          .then(res => res.json()) // parse response as JSON
          .then(data => {
            console.log(data)
            ...
            document.querySelector('.City').appendChild(h2)

            let h3 = document.createElement('h3')
            h3.innerText = 'Location: ' + location
            document.querySelector('.City').appendChild(h3)

            let feelsLike = data.main.feels_like
            console.log(feelsLike)
            let h4 = document.createElement('h4')
            h4.innerText = 'Temperature: ' + feelsLike + ' degrees Farenheit'
            document.querySelector('.City').appendChild(h4)

            let description = data.weather[0].description
            let h5 = document.createElement('h5')
            h5.innerText = 'Weather: ' + description
            document.querySelector('.City').appendChild(h5)
          })
      }
    })
}
